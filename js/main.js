...
function ennemi(stateAnimations, startingState, x, y, width, height, speed) {
    this.stateAnimations = stateAnimations; 
    this.currentState = startingState;
    this.x = 800; // position du bonhomme sur x
    this.y = y;
    this.width = width;
    this.height = height;
    this.speed = speed;
...
var spriteTiles = new Tileset('ennemi.png', 44, 108); // permet de donner de nouvelles valeurs à la function Tileset 
...
player.shootDelay = 300; // délai entre chaque balle
var projectiles = [];
...
function update(mod) {
	/* le si indique que Si une touché est pressé, le key code retourne true, si true il augmente/diminue la valeur des coordonnées x/y de notre sprite par la vitesse multiplier par le mod( qui permet de garder une framerate stable)*/
	 ...
	 if (81 in keysDown) {
        ennemi.currentState = 'left';
        ennemi.x -= ennemi.speed * mod;
        updateAnimation(ennemi.stateAnimations[ennemi.currentState]);
    }
    else if (68 in keysDown) {
        ennemi.currentState = 'right';
        ennemi.x += ennemi.speed * mod;
        updateAnimation(ennemi.stateAnimations[ennemi.currentState]);
    }
...
function render() {
	...
	drawSprite(ennemi);// affiche un enemi
